...
//        joinTable.executeInsert(DWD_COMMENT_INFO);
        ...
        tableEnv.sqlQuery("select " +
                        "`after`['id'] as id, " +  // 移除行内注释，仅保留必要逗号
                        "`after`['user_id'] as user_id, " +
                        "`after`['order_id'] as order_id, " +
                        "`after`['sku_id'] as sku_id, " +
                        "MD5(CAST(`after`['refund_type'] AS STRING)) as refund_type, " +
                        "`after`['refund_num'] as refund_num, " +
                        "`after`['refund_amount'] as refund_amount, " +
                        "MD5(CAST(`after`['refund_reason_type'] AS STRING)) as refund_reason_type, " +
                        "`after`['refund_reason_txt'] as refund_reason_txt, " +
                        "`after`['create_time'] as create_time " +  // 最后一个字段后无逗号
